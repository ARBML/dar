...
class KALIMAT(datasets.GeneratorBasedBuilder):
	def _info(self):
		return datasets.DatasetInfo(features=datasets.Features({'Article':datasets.Value('string'),'label': datasets.features.ClassLabel(names=['articlesLocal', 'articlesInternational', 'articlesSports', 'articlesCulture', 'articlesEconomy', 'articlesReligion'])}))
	...
	def _generate_examples(self, filepaths):
		...
		for i,filepath in enumerate(filepaths['inputs']):
			...
			df.columns = ['Article']
			df = df[['Article']]
			label = self.get_label_from_path(['articlesLocal', 'articlesInternational', 'articlesSports', 'articlesCulture', 'articlesEconomy', 'articlesReligion'], filepath.split('/')[-2])
