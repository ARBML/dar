...
class SenZi(datasets.GeneratorBasedBuilder):
	def _info(self):
		return datasets.DatasetInfo(features=datasets.Features({'tweet_filter':datasets.Value('string'),'arabizi':datasets.Value('string')}))
	...
	def _generate_examples(self, filepaths):
		...
		for i,filepath in enumerate(filepaths):
			...
			for _, record in df.iterrows():
				yield str(_id), {'tweet_filter':record['tweet_filter'],'arabizi':record['arabizi']}
				_id += 1 
