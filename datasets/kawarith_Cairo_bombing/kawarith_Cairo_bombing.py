...
class kawarith_Cairo_bombing(datasets.GeneratorBasedBuilder):
	def _info(self):
	...
	def _split_generators(self, dl_manager):
		...
		return [datasets.SplitGenerator(name=datasets.Split.TEST, gen_kwargs={'filepaths': [os.path.join(downloaded_files[0],f) for f in ['kawarith-main/Labelled Data/Cairo_bombing/Cairo_bombing_test.csv']]}),datasets.SplitGenerator(name=datasets.Split.TRAIN, gen_kwargs={'filepaths': [os.path.join(downloaded_files[0],f) for f in ['kawarith-main/Labelled Data/Cairo_bombing/Cairo_bombing_train.csv']]})]
