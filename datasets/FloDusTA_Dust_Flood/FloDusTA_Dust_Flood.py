...
class FloDusTA_Dust_Flood(datasets.GeneratorBasedBuilder):
	def _info(self):
		return datasets.DatasetInfo(features=datasets.Features({'id_str':datasets.Value('string'),'Unnamed: 1':datasets.Value('string'),'Unnamed: 2':datasets.Value('string'),'Unnamed: 3':datasets.Value('string'),'Annot1':datasets.Value('string'),'Annot2':datasets.Value('string'),'Annot3':datasets.Value('string'),'Unnamed: 7':datasets.Value('string'),'label': datasets.features.ClassLabel(names=['IRNE', 'IMFL', 'EXCLUDE', 'IMAC', 'IMDS'])}))
