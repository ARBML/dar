...
class FloDusTA_Dust_Traffic(datasets.GeneratorBasedBuilder):
	def _info(self):
		return datasets.DatasetInfo(features=datasets.Features({'id_str':datasets.Value('string'),'Unnamed: 1':datasets.Value('string'),'Unnamed: 2':datasets.Value('string'),'Unnamed: 3':datasets.Value('string'),'Annot1':datasets.Value('string'),'Annot2':datasets.Value('string'),'Annot3':datasets.Value('string'),'Unnamed: 7':datasets.Value('string'),'label': datasets.features.ClassLabel(names=['IRNE', 'IMAC'])}))
	def _split_generators(self, dl_manager):
		url = ['https://raw.githubusercontent.com/BatoolHamawi/FloDusTA/master/FloDusTA-Traffic%20Accident-Collection.csv']
