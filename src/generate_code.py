...
def get_generate_code(type, columns, labels, label_column_name, use_labels_from_path = False,
  ...
  if type == 'xlsx':
    ...
    if sep == "tab":
      pandas_df = TABS_3+f"df = pd.read_csv(open(filepath, 'rb'), sep = '\t', skiprows = {skiprows}, error_bad_lines = False, header = {header})\n"
    else:
  ...
  loop_body  = TABS_4+"yield str(_id), {" + ",".join([f"'{column}':record['{column}']" for column in columns if column != label_column_name])
